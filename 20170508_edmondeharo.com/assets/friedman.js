...
Site.Index = {
	...
	setSingleThumbDouble: function(thumb) {
		...
		if(Site.Index.useHiRes() && $img.data('4x') && $img.data('4x') !== $img.attr('src')) {

			$img.attr('src', $img.data('4x'));

		} else if(!Site.Index.useHiRes() && $img.data('hi-res') && $img.data('hi-res') !== $img.attr('src')) {
...
Site.Project = { 
	...
	resize : function() {
		...
		increment = Math.floor($('.navigation').height() / parseInt($('.navigation').css('line-height')));
